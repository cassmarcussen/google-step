...
public final class FindMeetingQueryTest {
  ...
  public void optionalAttendeeNotAvailable() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times. Add an optional attendee C who has an all-day event. 
  ...
  public void optionalAttendeeIsConsidered() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times. Add an optional attendee C who has an event between 8:30 and 9:00
  ...
  public void notEnoughRoomForOptionalAttendee() {
    // Have a person A, such that there is just enough room at one point in the day to
    // have the meeting. Add an optional attendee B who has an event between 8:30 and 8:45. 
  ...
  public void onlyOptionalAttendees() {
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0200PM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_A)),
        new Event("Event 3", TimeRange.fromStartDuration(TIME_0930AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 4", TimeRange.fromStartDuration(TIME_0130PM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_B)));
    ...
    Collection<TimeRange> expected =
        ...
        TimeRange.fromStartEnd(TIME_1000AM, TIME_0130PM, false));
  ...
  public void noOptionsForOnlyOptionalAttendees() {
    // Have no mandatory attendees, just two optional attendees with several 
    // gaps in their schedules. query should return that no time is available.
    //
    // Events  : |--A--|      |--A---|       |--A----|
    //                 |---B--|      |--B----|
    // Day     : |--------------------------------------|
  ...
  public void optionalAttendeeNotAvailableOptimizedMeetingTimes() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times. Add an optional attendee C who has an all-day event. Add another optional 
    // attendee who has an event from 8:30 to 9 am. Using the optimized meeting times
    // The same three time slots should be returned as when C was not invited.
    //
    // Events  :             |--A--|     |--B--|
    //           |--C--------|                 |--C--|
